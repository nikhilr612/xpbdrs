...
#[derive(Parser)]
#[command(name = "plane_surface_deformation")]
#[command(about = "XPBD plane surface deformation simulation with triangulated surface")]
#[command(version)]
struct Cli {
    ...
    /// Size of the deformable plane surface in world units
...
fn generate_plane_surface_mesh(resolution: usize, size: f32) -> TriangulatedSurface {
...
fn run_simulation(resolution: usize, size: f32) {
    ...
    let (mut rl, thread) = raylib::init()
        .size(800, 800)
        .title("XPBD Plane Surface Deformation Simulation")
        .build();
    ...
    info!(
        ...
        size = size,
        "Starting plane surface deformation simulation"
    );
    ...
    while !rl.window_should_close() {
        ...
        let acceleration_field = |vertex: &Vertex| {
            if vertex.inv_mass <= BOUNDARY_INV_MASS * 1.1 {
                ...
                {
                    let distance = (vertex.position - surface_center).length();
